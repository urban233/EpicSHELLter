...
Versioning
----------
EpicSHELLter uses a loose variant of semantic versioning (`SemVer`_) to govern deprecations,
API compatibility, and version numbering.

A EpicSHELLter release number is made up of ``MAJOR.MINOR.PATCH``.
...
EpicSHELLter will introduce deprecations in **minor** releases. These deprecations will preserve the existing behavior while emitting a warning that provide guidance on:

* How to achieve similar behavior if an alternative is available
* The EpicSHELLter version in which the deprecation will be enforced.

We will not introduce new deprecations in patch releases.

Deprecations will only be enforced in **major** releases.
For example, if a behavior is deprecated in EpicSHELLter 1.1.0,
...
Code style
----------
EpicSHELLter's code style is nearly completely based on the `Google Python Style Guide`_
